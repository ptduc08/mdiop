...
class ModelExtensionNews extends Model {
	public function addNews($data) {
		...
		//hoang ph  for category-new func
		if (isset($data['new_category'])) {
			foreach ($data['new_category'] as $category_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "new_to_category SET new_id = '" . (int)$news_id  . "', category_id = '" . (int)$category_id . "'");
			}
		}
		//end hoang ph
	}
	...
	public function editNews($news_id, $data) {
		...
		//hoangph -to edit ->delete old new_to_category and add new new_to_category  
		$this->db->query("DELETE FROM " . DB_PREFIX . "new_to_category WHERE new_id = '" . (int)$news_id . "'");
	...
	//hoangph get all category by of  new  by id
	public function getNewCategories($new_id) {
		$new_category_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "new_to_category WHERE new_id = '" . (int)$new_id . "'");

		foreach ($query->rows as $result) {
			$new_category_data[] = $result['category_id'];
		}
