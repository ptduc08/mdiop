    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-news" class="form-horizontal">
		  ...
		  <!-- hoangph for  category of new-->
		  <div class="form-group">
                <label class="col-sm-2 control-label" for="input-category"><span data-toggle="tooltip" title="<?php echo 'help_category'; ?>"><?php echo 'entry_category'; ?></span></label>
                <div class="col-sm-10">
                  <input type="text" name="category" value="" placeholder="<?php echo 'entry_category'; ?>" id="input-category" class="form-control" />
                  <div id="new-category" class="well well-sm" style="height: 150px; overflow: auto;">
                    <?php foreach ($new_categories as $new_category) { ?>
                    <div id="new-category<?php echo $new_category['category_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $new_category['name']; ?>
                      <input type="hidden" name="new_category[]" value="<?php echo $new_category['category_id']; ?>" />
                    </div>
                    <?php } ?>
                  </div>
                </div>
...
$('input[name=\'category\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=catalog/new_category/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
	...
	'select': function(item) {
		...
		$('#new-category' + item['value']).remove();

		$('#new-category').append('<div id="new-category' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="new_category[]" value="' + item['value'] + '" /></div>');
	}
});

$('#new-category').delegate('.fa-minus-circle', 'click', function() {
	$(this).parent().remove();
});
<!-- end hoangph  -->
</script>
